...
function keyDownHandler(event) {
  var a = myCamera.location;
  if (event.keyCode == 39 || event.keyCode == 68) {
    myCamera.location = [a[0] + 0.2, a[1], a[2]];
  }
  ...
  if (event.keyCode == 38 || event.keyCode == 87) {
    myCamera.location = [a[0], a[1], a[2] + 0.2];
  }
...
function mouseMoveHandler(event) {
  var a = myCamera.location;
  var witdh = gl.canvas.clientWidth;
  var height = gl.canvas.clientHeight;
  const factor = 2;
  const xpos = (event.offsetX - witdh / 2) / witdh
  const ypos = -(event.offsetY - height / 2) / height
  myCamera.location = [factor * xpos, factor * ypos, a[2]]
}
